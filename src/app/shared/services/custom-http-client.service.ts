...
export class CustomHttpClientService {
  ...
  delete<T>(requestParameter:Partial<RequestParameter>,id?:string):Observable<T>{
    ...
    if(requestParameter.fullEndPoint)
      ...
      url = `${this.createUrl(requestParameter)}${id ? `/${id}` : ""}${requestParameter.pathVariable ? `/${requestParameter.pathVariable}` : ""}${requestParameter.queryString ? `?${requestParameter.queryString}`:""}`
